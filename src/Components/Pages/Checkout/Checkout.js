...
const Checkout = () => {
    ...
    const [cuponLoad, setCuponLoad] = useState(false);
    const handleCouponSubmit = async () => {
        setCuponLoad(true);
        try {
            const response = await axios.get(`${getApiUrl()}/cupons/use?couponCode=${couponCode}`);
            setDiscount(response.data.discount);
        } catch (error) {
            if (error.response) {
                toast.error(error.response.data.message);
            } else {
                toast.error('Error occurred while using the coupon');
            }
        }
        setCuponLoad(false);
    };
    ...
    useEffect(() => {
        setTotal(subtotal + tax + shippingCharge - discount)
    ...
    const handlePlaceOrder = async () => {
        if (!paymentMethod) {
            ...
            if (JSON.stringify(shippingAddress) !== JSON.stringify(prevShippingAddress.current)) {
                try {
                    const response = await axios.put(`${getApiUrl()}/users/${userData.id}`, shippingAddress);
                    // Update the previous shipping address with the current one
                    prevShippingAddress.current = shippingAddress;
                } catch (error) {
                    console.error(error);
                }
            }
                ...
                <div className="w-full  lg:flex lg:space-x-[30px]">
                                                ...
                                                <div className="flex justify-between items-center">
                                                    <div>
                                                        <h4 className="text-[15px] text-qblack mb-2.5">
                                                            {product.name}<sup className="text-[13px] text-qgray ml-2 mt-2">x{product.orderQuantity}</sup>
                                                        </h4>
                                                        <p className="text-[13px] text-qgray">{product.brand_name}</p>
                                                    </div>
                                                    <div>
                    ...
                    <div className="flex-1"> 
                        ...
                        <h1 className="sm:text-2xl text-xl text-qblack font-medium mb-3">Summary</h1>
                            ...
                            <div>
                                <p className='ml-3'>Have a coupon code? Apply in this field and fill out!</p>
                                {
                                    cuponLoad && (
                                    ...
                                    <div className="w-full h-full flex items-center  border border-gray-border bg-white search-com">
                                        <div className="flex-1 bg-red-500 h-full">
                                            <form action="#" className="h-full"><input type="text"
                                                value={couponCode}
                                                onChange={handleCouponCodeChange} className="search-input" placeholder="Code" /></form>
                                        </div>
                                        <button onClick={handleCouponSubmit} className=" w-[93px] h-full text-sm font-600 primary-bg-color hover:bg-yellow-500" type="button">Apply</button></div>
                                </div>
                            </div>
