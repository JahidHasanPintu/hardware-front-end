...
import SmallProductCard from '../ProductCard/SmallProductCard/SmallProductCard';
...
const PopularProducts = (props) => {
    ...
    // const { products, totalPages } = useProducts(props.page, props.limit, props.search, props.brandID, props.catID, props.subcatID);
    if (loading) {
        return <Loading/>
    }
    ...
    return (
        <div className='section-wrapper w-11/12 mx-auto '>
            <div className='container-x mx-auto'>
                <div className=" section-title flex justify-between items-center my-4">
                    <div>
                        <h1 className="sm:text-3xl text-xl font-semibold text-qblacktext leading-none">{t('LatestProduct')}</h1>
                    </div>
                    <div>
                        <a href="/all-product">
                ...
                <div className='grid lg:grid-cols-5 md:grid-cols-5 sm:grid-cols-2 grid-cols-2 mt-2 justify-items-center relative'>

                    {
                        sortedProducts?.map(product => 
                            <SmallProductCard key={product.id}
                            product={product}/>
                        ).slice(0, 12)
                    }
